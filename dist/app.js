...
function renderTab(c,tab){
  ...
  if(tab==='video') root.innerHTML=`<article class="video-panel"><div class="eyebrow">Multimedia study</div><h2>Chapter video workshop</h2><div class="video-frame"><div><div class="play">▶</div><strong>Your NotebookLM lesson is in the course notebook</strong><br><small>Chapter videos are generated from the original book source.</small></div></div><h3>NotebookLM video brief</h3><div class="notebook-prompt" id="video-prompt">${c.videoPrompt}</div><div class="button-row"><a class="primary" href="https://notebook.google.com/notebook/63b7a110-ed59-49e4-b61e-5fd8ad08befe" target="_blank" rel="noopener">Open course notebook</a><button class="secondary" id="copy-video">Copy video brief</button><button class="secondary" id="download-packet">Download chapter packet</button></div><div class="key-idea"><strong>Recommended workflow</strong><br>Open the course notebook → Studio → Video Overview. Select the chapter-labeled video. Use this unit’s brief to regenerate or refine it whenever you want a different emphasis.</div><textarea id="chapter-notes" placeholder="Paste your takeaways or production notes here.">${state.notes[c.id]||''}</textarea><div class="button-row"><button class="primary" id="save-notes">Save notes</button></div></article>`;
  bindTab(c,tab);
}
