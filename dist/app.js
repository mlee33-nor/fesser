...
function renderTab(c,tab){
  ...
  if(tab==='quiz') root.innerHTML=`<article class="quiz-panel"><div class="eyebrow">Retrieval practice</div><h2>Chapter assessment</h2><p>Choose once before revealing the explanation. A score of 80% completes the fifth mastery check.</p>${c.quizzes.map((q,i)=>`<div class="quiz-question" data-q="${i}"><h3>${i+1}. ${q.q}</h3><div class="choices">${q.a.map((a,j)=>`<button class="choice" data-choice="${j}">${a}</button>`).join('')}</div><div class="explanation">${q.why}</div></div>`).join('')}<div class="button-row"><button class="primary" id="score-quiz">Score quiz</button><strong id="quiz-result"></strong></div></article>`;
  if(tab==='cards') root.innerHTML=`<article class="flashcard-panel"><div class="eyebrow">Active recall</div><h2>Concept flashcards</h2><p>Say the definition aloud before flipping.</p><div class="flashcard-deck">${c.vocabulary.map(([t,d])=>`<button class="flashcard"><strong class="prompt">${t}<br><small>Tap to reveal</small></strong><span class="answer">${d}</span></button>`).join('')}</div></article>`;
  ...
  if(tab==='video') root.innerHTML=`<article class="video-panel"><div class="eyebrow">Multimedia study</div><h2>Chapter video workshop</h2><div class="video-frame"><div><div class="play">▶</div><strong>Generate this chapter’s lesson in NotebookLM</strong><br><small>The packet and production brief are ready below.</small></div></div><h3>NotebookLM video brief</h3><div class="notebook-prompt" id="video-prompt">${c.videoPrompt}</div><div class="button-row"><button class="primary" id="copy-video">Copy video brief</button><button class="secondary" id="download-packet">Download chapter packet</button></div><div class="key-idea"><strong>Recommended workflow</strong><br>Upload the book or this chapter packet to NotebookLM → open Studio → choose Video overview → customize with the brief above → generate. When you have the share URL, paste it into your notes so the lesson stays connected to this unit.</div><textarea id="chapter-notes" placeholder="Paste your NotebookLM video link or production notes here.">${state.notes[c.id]||''}</textarea><div class="button-row"><button class="primary" id="save-notes">Save link / notes</button></div></article>`;
  bindTab(c,tab);
}
...
function bindTab(c,tab){
  ...
  if(tab==='quiz'){
    $$('.quiz-question').forEach(q=>$$('.choice',q).forEach(b=>b.onclick=()=>{if(q.dataset.selected!==undefined)return;q.dataset.selected=b.dataset.choice; b.classList.add(Number(b.dataset.choice)===c.quizzes[Number(q.dataset.q)].correct?'correct':'incorrect'); $$('.choice',q)[c.quizzes[Number(q.dataset.q)].correct].classList.add('correct'); $('.explanation',q).classList.add('show');}));
    $('#score-quiz').onclick=()=>{const qs=$$('.quiz-question');const right=qs.filter((q,i)=>Number(q.dataset.selected)===c.quizzes[i].correct).length;const score=Math.round(right/qs.length*100);state.scores[c.id]=Math.max(state.scores[c.id]||0,score);save();$('#quiz-result').textContent=`${score}% — ${score>=80?'passed':'review and retry'}`;toast(score>=80?'Mastery check complete':'Review the explanations, then retry');};
  }
